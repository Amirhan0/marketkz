...
error_reporting(0); // Отключение отчетов об ошибках
include('includes/config.php'); // Подключение конфигурационного файла

// Проверка, если сессия не активна (не авторизован), перенаправить на страницу логина
if (strlen($_SESSION['login']) == 0) {   
    header('location:login.php');
} else {
    // Обработка формы обновления информации
    if (isset($_POST['update'])) {
        $name = $_POST['name'];
        $contactno = $_POST['contactno'];
        $query = mysqli_query($con, "UPDATE users SET name='$name', contactno='$contactno' WHERE id='".$_SESSION['id']."'");
        if ($query) {
            echo "<script>alert('Ваша информация была обновлена');</script>";
        }
    }
...
<head>
    ...
    <!-- Только для демонстрации. Удалить в продакшн-версии -->
    ...
    function valid() {
        if (document.chngpwd.cpass.value == "") {
            alert("Поле 'Текущий пароль' пустое !!");
            document.chngpwd.cpass.focus();
            return false;
        } else if (document.chngpwd.newpass.value == "") {
            alert("Поле 'Новый пароль' пустое !!");
            ...
            alert("Пароль и поле 'Подтвердите пароль' не совпадают !!");
            document.chngpwd.cnfpass.focus();
            return false;
        }
        return true;
...
<!-- Для демонстрационных целей – можно удалить в продакшн-версии -->
